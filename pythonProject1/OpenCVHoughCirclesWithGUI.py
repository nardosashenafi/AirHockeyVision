...
minRadiusVar = 15  # Minimum radius for circle
maxRadiusVar = 30  # Max radius for circle
circleSensitivity = 100  # Sensitivity to circle detection
circleEdgePoints = 30  # Number of edge points necessary to declare a circle(more is a better circle)
...
def findCircles():
    ...
    dist = lambda x1, y1, x2, y2: (math.dist([x1, x2], [y1,
                                                        y2]))  # Function that calculates the square of the distance between two points in a frame
    ...
    while True:
        ...
        grayFrame = cv.cvtColor(undistortedFrame,
                                cv.COLOR_BGR2GRAY)  # Make a copy of frame where the color has been converted to grayscale
        # cv.imshow("Grayed Frame", grayFrame)				# (Can use different color settings, but cv.HoughCircles requires a grayscale image)

        blurFrame = cv.GaussianBlur(grayFrame, (blurLevel, blurLevel),
                                    0)  # Make a copy of grayFrame where the frame has been blurred
        ...
        if circles is not None:  # Are there circles detected in the frame
            ...
            for i in circles[0, :]:  # Iterate through array of circles
                ...
                if prevCircle is not None:  # Is there is a current circle stored
                    if dist(chosen[0], chosen[1], prevCircle[0], prevCircle[1]) \
                            <= dist(i[0], i[1], prevCircle[0], prevCircle[
                        1]):  # If the distance from the chosen circles center to the current circles center
            ...
            cv.circle(frame, (chosen[0], chosen[1]), 1, (0, 100, 100),
                      3)  # Draw a circle at the centerpoint of the chosen circle
            cv.circle(frame, (chosen[0], chosen[1]), chosen[2],
        ...
        cv.imshow("CameraVision",undistortedFrame)  # Show the original frame with the drawn circles to the user
        ...
        if cv.waitKey(1) & 0xFF == ord('q'):  # Quit program if user presses the 'q' key while in the imshow window
            ...
            print(
                f'Total Frames: {frameCounter:.0f}\n')  # Outputs the total number of frames processed by the detection program
            print(
                f'FPS: {(frameCounter / runtimeCounter):.3f}\n')  # Outputs the number of frames per second the detection program ran at
            print(
                f'Processing speed: {runtimeCounter / circleCounter:.3f} sec\n')  # Outputs the average time in seconds it took the program to process each circle
            print(
                f'Accuracy: {100 * (circleCounter / frameCounter):.3f}%')  # Outputs the ratio of circles found to number of frames processed in percentage
...
if __name__ == "__main__":
    ...
    #Thread(
    #    target=createGUI).start()  # Start the createGUI function in its own thread. Necessary to run simultaneously with findCircles function.
    Thread(
        target=findCircles).start()  # Start the findCircles function in its own thread. Necessary to run simultaneously with createGUI function.